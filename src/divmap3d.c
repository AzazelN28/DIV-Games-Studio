...
/**
 * Estructura para los archivos FPG. ฟEsto no molarํa mแs
 * importarlo de un .h? ฟPor qu้ redefinirlo aquํ?
 */
typedef struct {
  char magic1[3];
  char magic2[4];
...
/**
 * Informaci๓n de textura para el editor.
 *
 * ฟPor qu้ se definen cuatro?
 *
 * 1. Techo
 * 2. Suelo
...
// Estos valores por defecto para el editor de mapas 3D son muchํsimo
// mแs sensatos. Los antiguos valores eran:
// altura_techo = 2048;
// altura_suelo = 1024;
...
/**
 * Esta funci๓n dibuja el mini-mapa.
 */
void MapperVisor1(void)
{
  _show_items();
...
/**
 * Esta funci๓n gestiona lo que ocurre cuando se hace click
 * sobre la ventana del visor de mapas. Es decir, esta funci๓n
...
/**
 * Esta funci๓n es llamada cuando se destruye el visor de mapas.
 */
void MapperVisor3(void)
{
  int i;
...
void MapperCreator0(void)
{
  ...
  // ฟEsto no es un poco una redundancia? Ya se setea a 1 en
  // la definici๓n.
...
/**
 * Esta funci๓n es la encargada de dibujar el editor de mapas.
 */
void MapperCreator1(void)
{
  int an=v.an / big2,
...
*/
  ...
  switch(v.active_item)
  {
    ...
    case 3: // Flag de Sector
      ...
      need_refresh = 1;
      // ฟPor qu้ reordena los sectores?
      map_sortregions(1);
      map_off();
      break;
...
*/
  ...
  /**
   * Aquํ detectamos si se ha pulsado sobre alguno de los selectores
   * de textura/fondo.
   */
  if(mouse_b&1)
  {
 ...
 */
void M3D_set_flag(int flag) {
  // pone el modo de edici๓n igual al flag.
  modo_edicion = flag;
...
/**
 * Esta funci๓n renderiza las coordenadas.
 */
void mostrar_coordenadas(void)
{
  int an = v.an / big2,
...
void map_addpoint()
{
  ...
  // Si se ha presionado el bot๓n izquierdo del rat๓n.
  if (mouse_b & 1 && old_but1 == 0 || scan_code == _SPC) {
    ...
    if ((new_point = map_findpoint(pos_x, pos_y, -1)) < 0) {
      ...
      my_map->points[new_point]->links = 0; // ฟEnlaces? Supongo que esto debe indicar el n๚mero de paredes que enlazan a este punto.
    ...
    // indicamos que el nuevo punto estแ activo
    // ฟle a๑adimos 2 enlaces? WTF!?
    my_map->points[new_point]->active = 1;
    my_map->points[new_point]->links += 2;

    // indicamos que el ๚ltimo punto a๑adido es
    // este punto.
    ...
    else 
    {
      ...
      if (first_point == last_point && region_status > 2) {
        ...
        // ฟRestamos 2 a los enlaces del punto nuevo?
        my_map->points[new_point]->links-=2;

        // Obtenemos un nueva regi๓n del contador de regiones.
        new_region = my_map->num_regions;

        // Creamos una nueva regi๓n.
        ...
        my_map->regions[new_region]->type = 0; // ฟPara qu้ vale esto?
        ...
        // Incrementamos el n๚mero de regiones.
        my_map->num_regions++;

        // ฟPor qu้ se hacํa esto y ahora estแ comentado?
//        map_sortregions();
//
        //---------------------------------------------------------------------
  ...
  // Si se presiona el bot๓n derecho del rat๓n.
